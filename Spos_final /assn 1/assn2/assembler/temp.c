...
int main()
{
	...
	int opind = 0,lblind = 0,cnt = 0,counter =0,flg = 0,ind = 0,ind2 = 0;
	...
	while(1)
	{
		...
		if(strcmp(ims,"ORIGIN")==0)
		{
			...
			for(i = 0; i < index;i++)
			{
				if(strcmp(s[i].alabel[i],origin_val)==0)
				{
					if(flg==1)
		...
		else if(glflg==1)						//its a IS
		{
			...
			if(strcmp(op1,"#")==0 && strcmp(op2,"#")==0){									//for stop end
				...
				symin++;
				printf("%d\t(%s,%d)\t(%s,%d)\t%s\n",counter,"IS",isind,"S",symin,"#");
				fprintf(ic,"%d\t(%s,%d)\t(%s,%d)\t%s\n",counter,"IS",isind,"S",symin,"#");
			}
				...
				symin++;
				printf("%d\t(%s,%d)\t%d\t(%s,%d)\n",counter,"IS",isind,opindcc,"S",symin);
				fprintf(ic,"%d\t(%s,%d)\t%d\t(%s,%d)\n",counter,"IS",isind,opindcc,"S",symin);
			}
			else if(unknown==1 && unknown2==4 && unknownlit==0){	//reg label
				symin++;
				printf("%d\t(%s,%d)\t(%s,%d)\t(%s,%d)\n",counter,"IS",isind,"R",opindreg,"S",symin);
				fprintf(ic,"%d\t(%s,%d)\t(%s,%d)\t(%s,%d)\n",counter,"IS",isind,"R",opindreg,"S",symin);
			}
			else if(unknown==1 && unknownlit==7){	//reg litral
